...
class QuadrilateralShape{
    ...
    draw(){
        ...
        let angleABC = this.angle(a, b, c);
        let angleBCD = this.angle(b, c, d);
        let angleCDA = this.angle(c, d, a);
        let angleDAB = this.angle(d, a, b);
        let allAngle = [angleABC, angleBCD, angleCDA, angleDAB];
        let drawFlag = true;
        for(let i = 0; i < allAngle.length; i++){
            if(allAngle[i] != 45 || allAngle[i] != 90 || allAngle[i] != 135) drawFlag = false;
        }
        //if(drawFlag){
            if(this.isSquare()){
    ...
    // getAngle(angleStr){
    //     let angleA = angleStr[0];
    //     let angleB = angleStr[1];
    //     let angleC = angleStr[2];
    //     angleStr[0] == "A"
    // }
    ...
    isAllAngleNinety(){
        ...
        //console.log(isAngleABC90 + " " + isAngleBCD90 + " " + isAngleCDA90 + " " + isAngleDAB90);
    ...
    getShapeType(){
        ...
        if (this.isQuadrilateralShape()){ 
            // ねじれ四辺形
            if(this.isTwistedQuadrilateral()) return "other(その他）"
            // 平行な対辺の数
            ...
            if(numberOfParallelSides == 0){
                ...
                if (this.isSumAngleThreeHundredSixtyDegrees()) return "trapezoid(台形)";
                //else return "other（その他）";
            } else if(numberOfParallelSides == 2) {
                // 全ての辺の長さが等しい && 全ての角が直角
                if(this.isSquare()) return "square(正方形)";
                else if(this.isRhombus()) return "rhombus(ひし形)"; 
                else if(this.isRectangle()) return "rectangle(長方形)";
                else return "parallelogram(平行四辺形)";
            }
